...
char LeerRS232(void);
void EnviarDistancia(uint8_t duty);
...
void main(void) {
    ...
    if(POR==0)
    {
        POR=~POR;
        LCD_XY(0,0);
        LCD_Cadena("Quiere Retomar? ");
        while(1)
        {
            if (Tecla==16)
            {
                contados=EEPROM_Read(115);
                objetivo=EEPROM_Read(120);
                LCD_Comando(0x01);
                LCD_XY(0,0);
                char buffer[2];
                itoa(buffer,contados,10);
                LCD_Cadena(buffer);
                r=contados/10;
                i=contados-r*10;
                LATD=display[i];
    ...
    switch (seccion){ 
        ...
        case 3:
            ...
            LCD_Cadena("Faltan:");
            char buffer[2];
            itoa(buffer,(objetivo-contados),10);
            LCD_XY(1,13);
            LCD_Cadena(buffer);
            condicion=1;
            while(seccion==3){
...
void TransmitirCadena(const char *BufferT) {
    while (*BufferT) {  // Mientras no sea el car?cter nulo '\0'
        while (TXSTAbits.TRMT == 0); // Esperar a que el buffer de transmisi?n est? libre
        TXREG = *BufferT;  // Enviar car?cter
        BufferT++;  // Pasar al siguiente car?cter
    }
}
...
void ConfiguracionPWM(void)
{
    ...
    PR2=249;  /// Este registro es para configurar el periodo de la se?al
...
{
    EEADR=dir;
    EECON1bits.EEPGD=0; // UTILIZAR LA EEPROM
    EECON1bits.CFGS=0; // CONF EEPROM
...
void EEPROM_Write(char dato, char dir)
{
    GIE=0;
    EEDATA=dato;// IDENTIFICAR EL DATO
    EEADR=dir;// IDENTIFICAR LA DIRECCION DONDE SE VA A GUARDAR
    EECON1bits.EEPGD=0; // UTILIZAR LA EEPROM
    EECON1bits.CFGS=0; // CONF EEPROM
    EECON1bits.WREN=1; // HABILITAR ESCRITURA
    EECON2=0x55;  // En el datasheet debe estar especificado para la configuracion
    EECON2=0xAA;
    EECON1bits.WR=1; /// INICIAR LA ESCRITURA
    EECON1bits.WREN=0;  // DESHABIILITAR LA ESCRITURA
    while(EEIF==0);
    GIE=1;
}
void LeerRX_S(void)
